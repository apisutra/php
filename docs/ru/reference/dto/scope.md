...
Исходные формы JSON следуют за штатным выбором (`From`, fallback, unwrap, each),
но не за произвольными пользовательскими преобразованиями. Результаты cast/provider,
переопределённого computed, response handler, composite/EarlyReturn и вызовов
context->hydrate() — новый PHP-вход; SDK не угадывает происхождение сравнением
массивов. BeforeHydrate без возвращённой замены сохраняет формы, а возвращённый
массив создаёт новый вход, даже если выглядит прежним. Унаследованный
AbstractResponseDto::computed и штатный проходной hook запроса сохраняют формы.
Диагностический Boundary сам по себе не отключает проверку формы.
[Контракт JSON-формы](shapes.md#section-3).
